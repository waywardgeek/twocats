...
void test_output(TwoCats_HashType hashType,
                 ...
                 uint8_t memCost, uint8_t multiplies,
                 uint8_t lanes, uint8_t parallelism)
{
    ...
    if(!TwoCats_HashPasswordExtended(NULL, hashType, hash, pwd, pwdlen, salt, saltlen, data,
            datalen, memCost, memCost, multiplies, lanes, parallelism, TWOCATS_BLOCKSIZE,
...
void PHC_test(TwoCats_HashType hashType) {
    ...
    printf("****************************************** Test data\n");
    for(i=0; i < 256; i++) {
        test_output(hashType, NULL, 0, NULL, 0, (uint8_t *)&i, 1, TEST_MEMCOST,
            TWOCATS_MULTIPLIES, TWOCATS_LANES, TWOCATS_PARALLELISM);
    }
    printf("****************************************** Test memCost\n");
    for(i=0; i < TEST_MEMCOST; i++) {
        test_output(hashType, NULL, 0, NULL, 0, NULL, 0, i,
            TWOCATS_MULTIPLIES, TWOCATS_LANES, TWOCATS_PARALLELISM);
    }
    printf("****************************************** Test multiplies\n");
    for(i=0; i <= 8; i++) {
        test_output(hashType, NULL, 0, NULL, 0, NULL, 0, TEST_MEMCOST, i,
            TWOCATS_LANES, TWOCATS_PARALLELISM);
    }
    printf("****************************************** Test parallelism\n");
    for(i=1; i < 10; i++) {
        test_output(hashType, NULL, 0, NULL, 0, NULL, 0, TEST_MEMCOST,
...
void verifyPasswordUpdate(TwoCats_HashType hashType) {
    ...
    if(!TwoCats_HashPasswordExtended(NULL, hashType, hash1, password, 8,
    ...
    for(uint8_t memCost = 0; memCost < TEST_MEMCOST; memCost++) {
        memcpy(password, "password", 8);
        memcpy(salt, "salt", 4);
        if(!TwoCats_HashPasswordExtended(NULL, hashType, hash2, password, 8,
        ...
        if(!TwoCats_UpdatePassword(hashType, hash2, memCost + 1,
                TEST_MEMCOST, TWOCATS_MULTIPLIES, TWOCATS_LANES, TWOCATS_PARALLELISM,
                TWOCATS_BLOCKSIZE, TWOCATS_SUBBLOCKSIZE, false)) {
            fprintf(stderr, "Password hashing failed!\n");
            exit(1);
        }
...
void verifyClientServer(TwoCats_HashType hashType) {
    ...
    if(!TwoCats_ClientHashPassword(NULL, hashType, hash1, password, 8,

...
void PHC_test(TwoCats_HashType hashType) {
    ...
    for(i=0; i < TEST_MEMCOST; i++) {
        test_output(hashType, TWOCATS_KEYSIZE, NULL, 0, NULL, 0, NULL, 0, i, TWOCATS_TIMECOST,
            TWOCATS_MULTIPLIES, TWOCATS_LANES, TWOCATS_PARALLELISM);
    }
...
void verifyPasswordUpdate(TwoCats_HashType hashType) {
    ...
    if(!TwoCats_HashPasswordExtended(hashType, hash1, TWOCATS_KEYSIZE, (uint8_t *)"password", 8,
            ...
            TWOCATS_LANES, TWOCATS_PARALLELISM, TWOCATS_BLOCKSIZE, TWOCATS_SUBBLOCKSIZE, false, false)) {
        fprintf(stderr, "Password hashing failed!\n");
        exit(1);
    }

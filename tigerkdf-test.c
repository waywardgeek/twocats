...
void simpletest(char *password, char *salt, char *data, uint32_t memlen)
{
    test_output(64, (uint8_t *)password, strlen(password), (uint8_t *)salt, strlen(salt),
        (uint8_t *)data, strlen(data), memlen, 200, 0, 16384, 0, 1, 1);
}
...
void PHC_test(void)
{
    ...
    for(i=0; i < 256; i++) {
        test_output(32, (uint8_t *) &i, 1, &j, 1, NULL, 0, 1*1024, 200, 0, 16384, 0, 1, 1);
    }
    printf("****************************************** Test salt\n");
    for(i=0; i < 256; i++) {
        test_output(32, &j, 1, (uint8_t *) &i, 1, NULL, 0, 1*1024, 200, 0, 16384, 0, 1, 1);
    }
    printf("****************************************** Test data\n");
    for(i=0; i < 256; i++) {
        test_output(32, &j, 1, &j, 1, (uint8_t *) &i, 1, 10*1024, 200, 0, 16384, 0, 1, 1);
    }
    printf("****************************************** Test garlic\n");
    for(i=0; i < 6; i++) {
        test_output(32, &j, 1, &j, 1, NULL, 0, 1*1024, 200, i, 16384, 0, 1, 1);
    }
    printf("****************************************** Test parallelism\n");
    for(i=1; i < 10; i++) {
        test_output(32, &j, 1, &j, 1, NULL, 0, 1*1024, 200, 0, 16384, 0, i, 1);
    }
    printf("****************************************** Test repetitions\n");
    for(i=1; i < 10; i++) {
        test_output(32, &j, 1, &j, 1, NULL, 0, 1*1024, 200, 0, 16384, 0, 1, i);
    }
    ...
    printf("****************************************** Test memlen\n");
    for(i=1; i < 16; i += 4) {
        test_output(32, &j, 1, &j, 1, NULL, 0, i*1024, 200, 0, 16384, 0, 1, 1);
    }
}
...
void verifyGarlic(void) {
    ...
    if(!TigerKDF_HashPassword(hash1, 32, (uint8_t *)"password", 8,
            (uint8_t *)"salt", 4, 1*1024, 200, 0, NULL, 0, 16384, 0, 1, 1, false)) {
        fprintf(stderr, "Password hashing failed!\n");
        exit(1);
    }
    for(garlic = 1; garlic < 10; garlic++) {
        if(!TigerKDF_HashPassword(hash2, 32, (uint8_t *)"password", 8,
                (uint8_t *)"salt", 4, 1*1024, 200, garlic, NULL, 0, 16384, 0, 1, 1, false)) {
            fprintf(stderr, "Password hashing failed!\n");
            exit(1);
        }
        if(!TigerKDF_UpdatePasswordHash(hash1, 32, 1*1024, 200, garlic, garlic , 16384, 0, 1, 1)) {
            fprintf(stderr, "Password hashing failed!\n");
            exit(1);
        }
...
void verifyClientServer(void) {
    ...
    if(!TigerKDF_ClientHashPassword(hash1, 32, (uint8_t *)"password", 8, (uint8_t *)"salt",
            4, 1024*1024, 200, 0, (uint8_t *)"data", 4, 16384, 0, 2, 2, false)) {
        fprintf(stderr, "Password hashing failed!\n");
        exit(1);
    }
    ...
    if(!TigerKDF_HashPassword(hash2, 32, (uint8_t *)"password", 8, (uint8_t *)"salt", 4,
            1024*1024, 200, 0, (uint8_t *)"data", 4, 16384, 0, 2, 2, false)) {
        fprintf(stderr, "Password hashing failed!\n");
        exit(1);
    }
    if(memcmp(hash1, hash2, 32)) {
...
int main()
{
    ...
    test_output(128, (uint8_t *)"password", strlen("password"), (uint8_t *)"salt",
        strlen("salt"), NULL, 0, 1024*1024, 200, 0, 16384, 0, 2, 1);
    test_output(64, (uint8_t *)"password", strlen("password"), (uint8_t *)"salt",
        strlen("salt"), NULL, 0, 10*1024, 200, 0, 1024, 32, 1, 64);
    test_output(64, (uint8_t *)"password", strlen("password"), (uint8_t *)"salt",
